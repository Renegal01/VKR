...
import mouse  # Подключаем библиотеку mouse
...
def fine_tune_kalman_filter(gaze_estimator, kalman, camera_index=0):
    ...
    while len(points) > 0:
        ...
        if features is not None and not blink_detected:
            ...
            # Ограничение координат 1920x1080
            gaze_x = min(max(gaze_x, 0), 1920)
            gaze_y = min(max(gaze_y, 0), 1080)
...
def save_screenshot_on_left_click():
    screenshot = ImageGrab.grab()
    screenshot_path = os.path.join(input_dir, f"Screenshot_{int(time.time())}.png")
    screenshot.save(screenshot_path)
    with open("input/coordinates.txt", "a") as f:
        f.write(f"Screenshot saved at: {screenshot_path}\n")

# Устанавливаем обработчик нажатия левой кнопки мыши
mouse.on_click(save_screenshot_on_left_click)

def main():
    if os.path.exists("input/coordinates.txt"):
        os.remove("input/coordinates.txt")
    camera_index = 0
    ...
    # Отключаем обработчик клика после завершения программы
    mouse.unhook_all()
